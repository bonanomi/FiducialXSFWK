...
def plotXS(obsName, obs_bins, obs_bins_boundaries = False):
    ...
    if(acFlag):
        _temp = __import__('inputs_sig_ACggH_'+obsName+'_Full', globals(), locals(), ['acc'], -1)
        acc_AC = _temp.acc
    ...
    for obsBin in range(nBins-1):
        ...
        if(acFlag): ggH_AC[obsBin]+=XH_AC[obsBin]
    ...
    else: h_ratio_minloHJ.SetLineColor(ROOT.kRed+2)
    ...
    if acFlag: h_ggH_AC.Draw("histsame")
