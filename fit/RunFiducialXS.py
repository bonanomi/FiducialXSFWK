...
def produceDatacards(obsName, observableBins, ModelName, physicalmodel, process):
    print '\n'
        ...
        for fState in fStates:
            if (not obsName.startswith("mass4l")):
                for obsBin in range(nBins):
                    ...
                    createDatacard(obsName, fState, nBins, obsBin, observableBins, physicalmodel, year, ndata, JES, process)
                    os.chdir('../datacard/datacard_'+year)
            else:
                ndata = createXSworkspace(obsName,fState, nBins, 0, observableBins, False, True, ModelName, physicalmodel, year, JES, doubleDiff, process)
                createDatacard(obsName, fState, nBins, 0, observableBins, physicalmodel, year, ndata, JES, process)
                os.chdir('../datacard/datacard_'+year)
...
def runFiducialXS():
    ...
    # set process to ggH, same binning
    process = 'ggH'
    _temp = __import__('inputs_sig_'+obsName+'_'+process+'_'+opt.YEAR, globals(), locals(), ['observableBins'], -1)
    ...
    for process in processes:
        ...
        for year in years_bis:
            if not os.path.exists('../inputs/inputs_sig_'+obsName+'_'+process+'_'+year+'_ORIG.py'):
                cmd = 'python addConstrainedModel.py -l -q -b --obsName="'+obsName+'" --obsBins="'+opt.OBSBINS+'" --year="'+year+'" --procName="'+process+'"'
                if doubleDiff: cmd += ' --doubleDiff'
                print cmd
                output = processCmd(cmd)
                print output
                print 'addConstrainedModel DONE'
            elif os.path.exists('../inputs/inputs_sig_'+obsName+'_'+process+'_'+year+'_ORIG.py'):
        ...
        _temp = __import__('inputs_sig_'+obsName+'_'+process+'_'+opt.YEAR, globals(), locals(), ['acc'], -1)
        acc = _temp.acc
        ...
        for physicalModel in PhysicalModels:    
            produceDatacards(obsName, observableBins, DataModelName, physicalModel, process)
            ...
            for year in years:
                ...
                for fState in fStates:
                    if(nBins>0):
                        cmd = 'combineCards.py '
                        for obsBin in range(nBins):
                            cmd = cmd + 'hzz4l_'+fState+'S_13TeV_xs_'+obsName+'_bin'+str(obsBin)+'_'+physicalModel+'_'+process+'.txt '
                        cmd = cmd + '> hzz4l_'+fState+'S_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt'
                        print cmd, '\n'
                        processCmd(cmd,1)
                    else:
                ...
                cmd = 'combineCards.py hzz4l_4muS_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt hzz4l_4eS_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt hzz4l_2e2muS_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt > hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt'
                print cmd, '\n'
                processCmd(cmd,1)
            ...
            if (opt.YEAR == 'Full'):
                cmd = 'combineCards.py datacard_2016/hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt datacard_2017/hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt datacard_2018/hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt > hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt'
                print cmd, '\n'
                processCmd(cmd,1)
                cmd = 'echo "nuis group = CMS_eff_e CMS_eff_m CMS_hzz2e2mu_Zjets_2016 CMS_hzz2e2mu_Zjets_2017 CMS_hzz2e2mu_Zjets_2018 CMS_hzz4e_Zjets_2016 CMS_hzz4e_Zjets_2017 CMS_hzz4e_Zjets_2018 CMS_hzz4mu_Zjets_2016 CMS_hzz4mu_Zjets_2017 CMS_hzz4mu_Zjets_2018 QCDscale_VV QCDscale_ggVV kfactor_ggzz lumi_13TeV_2016 lumi_13TeV_2017 lumi_13TeV_2018 norm_fakeH pdf_gg pdf_qqbar CMS_zz4l_sigma_e_sig_2017 CMS_zz4l_sigma_e_sig_2016 CMS_zz4l_sigma_m_sig_2018 CMS_zz4l_sigma_m_sig_2017 CMS_zz4l_sigma_m_sig_2016 CMS_zz4l_n_sig_3_2016 CMS_zz4l_n_sig_3_2017 CMS_zz4l_mean_e_sig_2016 CMS_zz4l_mean_e_sig_2017 CMS_zz4l_n_sig_3_2018 CMS_zz4l_mean_m_sig_2018 CMS_zz4l_mean_m_sig_2016 CMS_zz4l_mean_m_sig_2017 CMS_zz4l_sigma_e_sig_2018 CMS_zz4l_mean_e_sig_2018" >> hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt'
                print cmd, '\n'
                processCmd(cmd,1)
            else:
                cmd = 'cp datacard_'+str(opt.YEAR)+'/hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt'
                print cmd, '\n'
                processCmd(cmd,1)
                cmd = "sed -i 's|hzz4l|datacard_"+str(opt.YEAR)+"/hzz4l|g' hzz4l_all_13TeV_xs_"+obsName+"_bin_"+physicalModel+'_'+process+".txt" # Specify the right pattern to datacards (Before it was not necessary because there was a further combination)
                print cmd, '\n'
                processCmd(cmd,1)
                cmd = 'echo "nuis group = CMS_eff_e CMS_eff_m CMS_hzz2e2mu_Zjets_'+str(opt.YEAR)+' CMS_hzz4e_Zjets_'+str(opt.YEAR)+' CMS_hzz4mu_Zjets_'+str(opt.YEAR)+' QCDscale_VV QCDscale_ggVV kfactor_ggzz lumi_13TeV_'+str(opt.YEAR)+' norm_fakeH pdf_gg pdf_qqbar CMS_zz4l_sigma_e_sig_'+str(opt.YEAR)+' CMS_zz4l_sigma_m_sig_'+str(opt.YEAR)+' CMS_zz4l_n_sig_3_'+str(opt.YEAR)+' CMS_zz4l_mean_e_sig_'+str(opt.YEAR)+' CMS_zz4l_mean_m_sig_'+str(opt.YEAR)+' CMS_zz4l_sigma_e_sig_'+str(opt.YEAR)+'" >> hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt'
                processCmd(cmd,1)
                print cmd, '\n'
            ...
            if (physicalModel=="v3"):
                ...
                cmd = 'text2workspace.py hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.txt -P HiggsAnalysis.CombinedLimit.HZZ4L_Fiducial:trilinearHiggs -o hzz4l_all_13TeV_xs_'+obsName+'_bin_'+physicalModel+'_'+process+'.root'
                print cmd, '\n'
                processCmd(cmd)
